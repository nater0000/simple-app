...
size_t webui_bind(size_t window, const char* element, void( * func)(webui_event_t* e)) {
    ...
    // [win num][/][element]
    char* webui_internal_id = (char*)_webui_malloc(3 + 1 + len);
...
bool webui_set_tls_certificate(const char* certificate_pem, const char* private_key_pem) {
    ...
    if (!_webui_is_empty(certificate_pem) && !_webui_is_empty(private_key_pem)) {
        ...
        _webui_core.ssl_cert = (uint8_t*)ssl_cert;
        _webui_core.ssl_key = (uint8_t*)ssl_key;
...
void webui_get_window_size(size_t window, unsigned int* width, unsigned int* height) {

#ifdef WEBUI_LOG
    printf("[User] webui_get_window_size(%zu)...\n", window);
#endif
...
static bool _webui_show_window(_webui_window_t * win, const char* content, int type, size_t browser) {
    ...
    #ifdef WEBUI_TLS
    // TLS
    if (_webui_is_empty((char*)_webui_core.ssl_cert) || _webui_is_empty((char*)_webui_core.ssl_key)) {
        ...
        _webui_core.root_cert = (uint8_t*)root_cert;
        _webui_core.root_key = (uint8_t*)root_key;
        _webui_core.ssl_cert = (uint8_t*)ssl_cert;
        _webui_core.ssl_key = (uint8_t*)ssl_key;
    ...
    if (type == WEBUI_SHOW_HTML) {
        ...
        // Set window URL
        // @FIX: The caller may free this memory
...
static int _webui_system_win32_out(const char* cmd, char ** output, bool show) {
    ...
    while(ReadFile(stdout_read, buffer, WEBUI_STDOUT_BUF, & bytes_read, NULL) && bytes_read > 0) {
        ...
        char* new_output = (char*)realloc( * output, output_size + bytes_read + 1);
        if (new_output == NULL) {
            free( * output);
            CloseHandle(stdout_read);
