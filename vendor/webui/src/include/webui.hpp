...
class window {
    ...
    class event : public webui_event_t {
        ...
        class handler {
            ...
            private:
            static std::array<callback_t, 512> callback_list;

            // List of window objects: webui::window
            static std::array<webui::window*, 512> window_list;
    ...
    // Get window size
    void get_window_size(unsigned int* width, unsigned int* height) const { webui_get_window_size(webui_window, width, height); }

    // Set window size
    void set_window_size(unsigned int width, unsigned int height) const { webui_set_size(webui_window, width, height); }
    ...
    size_t get_server_port() const { return webui_get_port(webui_window); }
    ...
    void set_server_port(size_t port) const { webui_set_port(webui_window, port); }
...
std::array<window::event::handler::callback_t, 512> window::event::handler::callback_list {};
std::array<webui::window*, 512> window::event::handler::window_list {};
